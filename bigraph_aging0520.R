#Calculation
#source("/Users/dokada/Dropbox/analysis/2022.5/bigraph_aging0520.R") #Run in 2024.0520
out_path <- "/Users/dokada/Desktop/work/bigraph_aging0520/"
...
for(ds in 1:2){
    ...
    age_all_bin <- age_all
    age_all_bin[age_all == 3] <- "Young"
    age_all_bin[age_all >= 18] <- "Old"
    sex_all <- sample_annot$sex
    tissue_all <- sample_annot$tissue
    celltype_all <- sample_annot$cell_ontology_class
    stca_all <- paste0(sex_all, ".", tissue_all, ".", celltype_all, ".", age_all_bin)
    stca_all2 <- stca_all[age_all_bin %in% c("Young", "Old")]


    tab <- table(stca_all2)
    used_stc <- names(tab)[tab > 25]
    ...
    age <- sapply(strsplit(used_stc, "\\."), function(x){x[4]})
    dat <- data.frame(sex, tissue, celltype, age)
    edge_file <- paste0(out_path, tmp_ds, "celltissue_young.csv")
